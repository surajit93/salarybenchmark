...
CONFIG_CHANGED = PAGE_INDEX.get("config_version") != CONFIG.get("version")
if CONFIG_CHANGED:
    PAGE_INDEX["pages"] = {}
...
def build_salary_data_json():
    ...
    for state_id, state_data in SALARY["by_state"].items():
        out[state_id] = {
            k: v for k, v in state_data.items() if k not in ("name", "abbr", "cost_index")
        }
...
def build_related_pages(bucket, age_band, state_id=None):
    ...
    idx = (
        bands.index(age_band["id"])
        if age_band in AGE_BANDS
        else next((i for i, b in enumerate(AGE_BANDS) if b["id"] == age_band["id"]), 0)
    )
...
def generate_page(bucket, age_band, state_id=None):
    ...
    cost_index = data.get("cost_index") if isinstance(data, dict) else None
    ...
    related_pages = build_related_pages(bucket, age_band, state_id)
    related_pages = related_pages if "related_pages" in locals() else []
    percentile_range = "50th–70th"
...
def build_sitemap(pages):
    ...
    for url in pages:
        lines.append(
            f"  <url><loc>{base}{url}</loc><changefreq>monthly</changefreq><priority>0.8</priority></url>"
        )
    lines.append("</urlset>")
...
def write_robots_txt():
    ...
    robots_content = (
        "User-agent: *\n"
        "Allow: /\n"
        f"Sitemap: {CONFIG['domain'].rstrip('/')}/sitemap.xml\n"
    )
    robots_path.write_text(robots_content, encoding="utf-8")


def write_root_index_if_missing():
    index_path = OUT_DIR / "index.html"
    if not index_path.exists():
        index_path.write_text(
            "<h1>Benchmark Your Financial Position</h1>\n"
            "<p>Compare your salary, savings, and net worth anonymously.</p>\n",
            encoding="utf-8",
        )


def persist_page_index():
...
if __name__ == "__main__":
    ...
    build_sitemap(pages)
    write_robots_txt()
    write_root_index_if_missing()
    persist_page_index()

    print(f"\n✓ Generated {len(pages)} pages → {OUT_DIR}")
    print("✓ Sitemap written")
    print("✓ Robots + page index updated")
